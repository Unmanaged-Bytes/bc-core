...
static bool bc_core_sort_insertion(unsigned char* base, size_t count, size_t element_size, bc_core_sort_less_than_function less_than,
                                   void* user_data)
{
    for (size_t outer = 1; outer < count; ++outer) {
        ...
        while (inner > 0) {
            ...
            if (!bc_core_swap(current, previous, element_size)) {
                return false;
            }
            --inner;
        }
...
static bool bc_core_sort_sift_down(unsigned char* base, size_t start, size_t end, size_t element_size,
                                   bc_core_sort_less_than_function less_than, void* user_data)
{
    ...
    while ((root * 2U) + 1U <= end) {
        ...
        if (!bc_core_swap(root_pointer, child_pointer, element_size)) {
            return false;
        }
        root = child;
    }
...
static bool bc_core_sort_heap(unsigned char* base, size_t count, size_t element_size, bc_core_sort_less_than_function less_than,
                              void* user_data)
{
    if (count <= 1U) {
        return true;
    ...
    while (start_plus_one > 0U) {
        ...
        if (!bc_core_sort_sift_down(base, start, count - 1U, element_size, less_than, user_data)) {
            return false;
        }
        --start_plus_one;
    }
    ...
    while (end > 0U) {
        ...
        if (!bc_core_swap(base, end_pointer, element_size)) {
            return false;
        }
        --end;
        if (!bc_core_sort_sift_down(base, 0U, end, element_size, less_than, user_data)) {
            return false;
        }
    }
    return true;
...
static bool bc_core_sort_introsort(unsigned char* base, size_t count, size_t element_size, bc_core_sort_less_than_function less_than,
                                   void* user_data, size_t depth_limit)
{
    while (count > BC_CORE_SORT_INSERTION_THRESHOLD) {
        if (depth_limit == 0U) {
            return bc_core_sort_heap(base, count, element_size, less_than, user_data);
        }
        depth_limit -= 1U;

        size_t pivot_index = count / 2U;
        unsigned char* last = base + (count - 1U) * element_size;
        if (!bc_core_swap(base + pivot_index * element_size, last, element_size)) {
            return false;
        }
        ...
        for (size_t scan = 0; scan < count - 1U; ++scan) {
            ...
            if (less_than(candidate, last, user_data)) {
                ...
                if (!bc_core_swap(candidate, destination, element_size)) {
                    return false;
                }
                store += 1U;
            }
        ...
        if (!bc_core_swap(pivot_final, last, element_size)) {
            return false;
        }
        ...
        if (left_count < right_count) {
            if (!bc_core_sort_introsort(base, left_count, element_size, less_than, user_data, depth_limit)) {
                return false;
            }
            base = right_base;
            count = right_count;
        } else {
            if (!bc_core_sort_introsort(right_base, right_count, element_size, less_than, user_data, depth_limit)) {
                return false;
            }
            count = left_count;
        }
    ...
    return bc_core_sort_insertion(base, count, element_size, less_than, user_data);
}
...
bool bc_core_sort_with_compare(void* base, size_t count, size_t element_size, bc_core_sort_less_than_function less_than, void* user_data)
{
    ...
    return bc_core_sort_introsort((unsigned char*)base, count, element_size, less_than, user_data, depth_limit);
}
